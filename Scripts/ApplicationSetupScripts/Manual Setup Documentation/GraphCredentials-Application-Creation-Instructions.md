# Manual Graph Credentials Azure AD Application Creation Instructions

This document provides step-by-step instructions for manually creating the Graph Credentials Azure AD application that is typically created by the `Set-GraphCredentialsAzureADApplication.ps1` script.

## Overview

The Graph Credentials application serves as the primary service application for the Group Membership Management (GMM) system. It provides high-privilege access to Microsoft Graph API for reading user information, managing group memberships, and performing core GMM operations. This application uses both application permissions (for unattended operations) and delegated permissions (for specific scenarios like email sending and Teams channel management).
...
- Understanding of Microsoft Graph permissions and their implications
...
3. Configure the basic settings:
   - **Name**: `{SolutionAbbreviation}-Graph-{EnvironmentAbbreviation}`
...
4. Add the redirect URIs (you might have to input the first one, click configure, and then input the others):
   - `https://{SolutionAbbreviation}-compute-{EnvironmentAbbreviation}-graphupdater.azurewebsites.net`
   - `https://{SolutionAbbreviation}-compute-{EnvironmentAbbreviation}-groupmembershipobtainer.azurewebsites.net`
   - `http://localhost` (for local development/testing)
   - Examples:
     - `https://gmm-compute-prod-graphupdater.azurewebsites.net`
     - `https://gmm-compute-prod-groupmembershipobtainer.azurewebsites.net`
     - `http://localhost`
5. Click **Configure**
...
#### 5.1 Add Microsoft Graph Application Permissions
...
#### 5.2 Add Microsoft Graph Delegated Permissions
...
3. Verify all permissions show green checkmarks with "Granted for [Your Organization]"
...
3. Configure:
   - **Description**: `GMM Graph Credentials Secret`
...
4. Search for your application name: `{SolutionAbbreviation}-Graph-{EnvironmentAbbreviation}`
   - Example: `gmm-Graph-prod`
...
#### Basic Properties
- [ ] **Display Name**: `{SolutionAbbreviation}-Graph-{EnvironmentAbbreviation}`
- [ ] **Sign-in Audience**: `AzureADMyOrg`
- [ ] **Public Client**: Enabled (`isFallbackPublicClient: true`)

#### Authentication Configuration
- [ ] **Web Platform**: Configured with redirect URIs:
  - [ ] `https://{solution}-compute-{env}-graphupdater.azurewebsites.net`
  - [ ] `https://{solution}-compute-{env}-groupmembershipobtainer.azurewebsites.net`
  - [ ] `http://localhost`
...
- [ ] **Microsoft Graph Application Permissions**:
  ...
  - [ ] `Member.Read.Hidden` with admin consent
- [ ] **Microsoft Graph Delegated Permissions**:
  - [ ] `ChannelMember.ReadWrite.All` with admin consent
  - [ ] `Mail.Send` with admin consent
...
### Issue: "Permission not granted for application permissions"
- **Solution**: Verify admin consent was granted for all application permissions
- **Check**: API permissions tab shows green checkmarks for all high-privilege permissions

### Issue: "Authentication fails in Graph operations"
- **Solution**: Verify the application is configured as a public client and has correct redirect URIs
- **Check**: "Allow public client flows" is set to "Yes" and redirect URIs match deployed services

### Issue: "Access denied when reading group memberships"
- **Solution**: Ensure GroupMember.Read.All and Member.Read.Hidden permissions are granted
- **Check**: Application permissions are properly consented by administrator

### Issue: "Application secret expired"
- **Solution**: Create a new client secret and update GMM Key Vault configuration
...
### Issue: "Teams channel operations failing"
- **Solution**: Verify ChannelMember.ReadWrite.All delegated permission is granted
- **Check**: Teams-specific operations require proper user context

---

**Note**: These instructions are based on the configuration created by `Set-GraphCredentialsAzureADApplication.ps1`. For automated setup, use the PowerShell script instead of manual configuration.