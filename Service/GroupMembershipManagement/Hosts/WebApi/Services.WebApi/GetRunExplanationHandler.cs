...
Use only the provided data. Output 1-2 sentences normally, or up to 3 sentences when the ""Per-part attribution"" section names two or more sources (so every cited source can be included).";
        ...
        protected override async Task<GetRunExplanationResponse> ExecuteCoreAsync(GetRunExplanationRequest request)
        {
            ...
            try
            {
                ...
                // Detect threshold-blocked runs from the run status — the disable-cap path leaves Status=ThresholdExceeded.
                ...
                var isThresholdBlocked = string.Equals(runHistory.Status, "ThresholdExceeded", StringComparison.OrdinalIgnoreCase);
                ...
                var userPrompt = BuildRunPrompt(request, runHistory, membershipRules, cappedAdded, cappedRemoved, added.Count, removed.Count, isThresholdBlocked, isInitialRun, configDiff, hrDiff, groupNames, managerNames, addsAttribution, removesAttribution, ClassifyJobSourceKind(parts));
