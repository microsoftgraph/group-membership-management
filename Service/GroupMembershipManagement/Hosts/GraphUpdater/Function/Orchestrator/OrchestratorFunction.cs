        ...
        public async Task<OrchestrationRuntimeStatus> RunOrchestratorAsync([OrchestrationTrigger] TaskOrchestrationContext context)
        {
            ...
            try
            {
                ...
                if (groupId.Equals(Guid.Empty))
                {
                    ...
                    await context.CallActivityAsync(nameof(JobStatusUpdaterFunction), CreateJobStatusUpdaterRequest(syncJob, SyncStatus.Error));
                ...
                await context.CallActivityAsync(nameof(JobStatusUpdaterFunction),
                                    CreateJobStatusUpdaterRequest(syncJob,
                                                                    SyncStatus.Idle, membersAddedResponse.SuccessCount, membersRemovedResponse.SuccessCount));
