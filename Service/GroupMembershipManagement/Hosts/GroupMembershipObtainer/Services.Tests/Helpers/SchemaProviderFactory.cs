...
namespace Tests.Helpers
{
    internal static class SchemaProviderFactory
    {
        ...
        public static SchemaProvider CreateMissingGroupMembershipSchemaProvider()
        {
            var jsonSchemaProvider = new SchemaProvider();
            var schemaFolderName = "Schemas";
            ...
            if (jsonSchemaDirectory != null && Directory.Exists(jsonSchemaDirectory))
            {
                ...
                {
                    var fileName = Path.GetFileNameWithoutExtension(file);
                    if (fileName != "GroupMembershipSchema") jsonSchemaProvider.Schemas.Add((Schema)Enum.Parse(typeof(Schema), fileName), File.ReadAllText(file));
                }
            }
