...
namespace Hosts.JobTrigger
{
    public class GetJobsFunction
    {
        ...
        public async Task<List<SyncJob>> GetJobsToUpdateAsync([ActivityTrigger] object obj)
        {
            ...
            {
                await _loggingRepository.LogMessageAsync(new LogMessage { Message = $"{nameof(GetJobsFunction)} function is not proceeding with {tableQuerySegment.Count} jobs due to JobTrigger threshold limit exceed." }, VerbosityLevel.DEBUG);
                return new List<SyncJob>();
            }

            await _loggingRepository.LogMessageAsync(new LogMessage { Message = $"{nameof(GetJobsFunction)} number of jobs about to be returned: {tableQuerySegment.Count}" }, VerbosityLevel.DEBUG);
