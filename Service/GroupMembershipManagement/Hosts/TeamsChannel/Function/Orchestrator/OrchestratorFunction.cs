    ...
    public class OrchestratorFunction
    {
        ...
        public async Task RunOrchestratorAsync([OrchestrationTrigger] IDurableOrchestrationContext context, ExecutionContext executionContext)
        {
            ...
            try
            {
                ...
                var parsedAndValidated = await context.CallActivityAsync<(AzureADTeamsChannel parsedChannel, bool isGood)>(nameof(ChannelValidatorFunction), channelSyncInfo);
                ...
                var filePath = await context.CallActivityAsync<string>(nameof(UserUploaderFunction),
                     new UserUploaderRequest
                     {
                         Users = users,
                         ChannelSyncInfo = channelSyncInfo,
