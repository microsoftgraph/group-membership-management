...
param servicePlanName string = '${solutionAbbreviation}-${resourceGroupClassification}-${environmentAbbreviation}-${substring(uniqueString(subscription().id,'AutoApprover'),0,8)}'
...
module servicePlanTemplate 'servicePlan.bicep' = {
  name: 'servicePlanTemplate-AutoApprover'
  params: {
...
module userAssignedManagedIdentityNameReader 'keyVaultReader.bicep' = {
  name: 'uamiNameReader-AutoApprover'
  params: {
...
module existingLogAnalyticsWorkspace 'logAnalyticsWorkspace.bicep' = {
  name: 'existingLogAnalyticsWorkspace-auto'
