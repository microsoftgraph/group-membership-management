    ...
    public class MessageProcessorOrchestrator
    {
        ...
        public async Task RunMessageProcessorAsync([OrchestrationTrigger] IDurableOrchestrationContext context)
        {
            try
            {
                ...
                var messageEntityId = new EntityId(MessageEntity.EntityName, messageId);
                ...
                if (request != null)
                {
                    await context.CallActivityAsync(nameof(LoggerFunction), new LoggerRequest { Message = $"Calling {nameof(OrchestratorFunction)} for jobId {request.SyncJob.Id}", RunId = request.SyncJob.RunId.GetValueOrDefault() });
