    ...
    public class OrchestratorFunction
    {
            ...
            [OrchestrationTrigger] TaskOrchestrationContext context)
        {
            ...
            try
            {
                ...
                // Apply membership changes sequentially and in order: add first, then remove. Adds and
                // removes to the same Teams channel share a single ~1 request/second-per-channel Microsoft
                // Graph throttling bucket, so running the two phases sequentially (rather than concurrently)
                // avoids self-contention/429s on the channel and keeps a deterministic, in-order application
                // of the membership delta. Cross-job parallelism (multiple channels at once) is bounded
