...
namespace Repositories.SecurityGroupCrawler.Tests
{
	...
	public class SecurityGroupCrawlerTests
	{
		...
		[TestMethod]
		public async Task FindsUsersWithCyclesNotInHeadGroup()
		{
			var collector = new CrawlResultCollector();
			var groups = MakeSingleCycleDictionary();
			var crawler = collector.Attach(new SGCrawler(new MockSGRepository(groups, TimeSpan.FromSeconds(0))));

			await crawler.CrawlGroup(_help.GroupNamed(0));

			CollectionAssert.AreEquivalent(new[] { _help.UserNamed(0), _help.UserNamed(1), _help.UserNamed(2), _help.UserNamed(3), _help.UserNamed(4), _help.UserNamed(5) },
				collector.Users.Distinct().ToArray());
			CollectionAssert.AreEquivalent(new[] { _help.GroupNamed(0), _help.GroupNamed(1), _help.GroupNamed(2), _help.GroupNamed(3), _help.GroupNamed(4) },
				collector.Groups.ToArray());
			var foundcycle = collector.Cycles.Single();
			Assert.AreEqual(_help.GroupNamed(1), foundcycle.Group);

			// equal here because order matters
			CollectionAssert.AreEqual(new[] { _help.GroupNamed(1), _help.GroupNamed(3), _help.GroupNamed(4) }, foundcycle.Cycle);
		}
		...
		private Dictionary<AzureADGroup, IEnumerable<IAzureADObject>> MakeSingleCycleDictionary()
		{
			return new Dictionary<AzureADGroup, IEnumerable<IAzureADObject>>
			{
				{ _help.GroupNamed(0), new IAzureADObject[] { _help.GroupNamed(1), _help.UserNamed(0) }  },
				{ _help.GroupNamed(1), new IAzureADObject[] { _help.GroupNamed(2), _help.GroupNamed(3), _help.UserNamed(0), _help.UserNamed(1), _help.UserNamed(2) }  },
				{ _help.GroupNamed(2), new IAzureADObject[] { _help.UserNamed(3) }  },
				{ _help.GroupNamed(3), new IAzureADObject[] { _help.UserNamed(4), _help.GroupNamed(4) }  },
				{ _help.GroupNamed(4), new IAzureADObject[] { _help.UserNamed(5), _help.GroupNamed(1) }  },
			};
		}
